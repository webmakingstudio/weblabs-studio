...
const LoadingAnimation = ({ progress }: { progress: number }) => (
  <div className="relative w-6 h-6">
    <svg viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full" aria-label={`Loading progress: ${Math.round(progress)}%`} >
...
export default function AILoadingState() {
  ...
  const lineHeight = 32; // Aumentado para mejor separación
  const currentSequence = TASK_SEQUENCES[sequenceIndex];
  const totalLines = currentSequence.lines.length;

  useEffect(() => {
    const initialLines = [];
    for (let i = 0; i < Math.min(3, totalLines); i++) { // Mostrar solo 3 líneas
      initialLines.push({
  ...
  useEffect(() => {
    const advanceTimer = setInterval(() => {
      ...
      const nextLineIndex = (firstVisibleLineIndex + 2) % totalLines; // Avanzar más lento
    ...
    }, 2500); // Más lento para mejor legibilidad
  ...
  return (
    <div className="flex items-center justify-center min-h-full w-full">
      <div className="space-y-4 w-auto">
        <div className="ml-2 flex items-center space-x-2 text-zinc-900 dark:text-white font-medium">
        ...
        <div className="relative">
          <div
            ...
            className="font-mono text-xs overflow-hidden w-full h-[120px] relative rounded-lg" // Altura reducida para 3 líneas
            style={{ scrollBehavior: "smooth" }}
          >
            <div>
              {visibleLines.map((line, index) => (
                <div key={`${line.number}-${line.text}`} className="flex h-[32px] items-center px-2 py-1">
                  <div className="text-zinc-500 dark:text-zinc-400 pr-3 select-none w-6 text-right">
                    {line.number}
                  </div>
                  <div className="text-zinc-900 dark:text-white flex-1 ml-1">
                    {line.text}
                  </div>
                </div>
              ))}
          ...
          <div className="absolute top-0 left-0 right-0 bottom-0 pointer-events-none rounded-lg from-white/90 via-white/50 to-transparent dark:from-black/90 dark:via-black/50 dark:to-transparent" style={{ background: "linear-gradient(to bottom, var(--tw-gradient-from) 0%, var(--tw-gradient-via) 30%, var(--tw-gradient-to) 100%)" }} />
        </div>
      </div>
    </div>
  );
